...
void echo_input_word(fp, word, rtcode, mode)
FILE *fp;	/* output file */
...
*/
void put_tab_or_plus_mark(fp, flag, plusflag)
FILE *fp;	/* output file */
...
*/
void put_morph(fp, flag, morph, pos, mode)
FILE *fp;	/* output file */
...
*/
void put_morph_result(fp, pr, mode)
FILE *fp;	/* output file */
HAM_PRESULT pr;	/* morph. analysis result */
HAM_PRUNMODE mode;
{
...
*/
int hamout2tokenList(p, toklist, tokendelim)
HAM_PMORES p;
...
char tokendelim;	/* 토큰 구분 문자 */
{
	...
	for (i=0; i < p->nword; i++) {
		...
		wordleng = strlen(pw->phon);
		stemleng = strlen(pr->stem);
		if (stemleng > wordleng) stemleng = wordleng;

		if (i) *out++ = ' ';
		strncpy(out, word, stemleng); n++;
		out += stemleng;

		if (wordleng > stemleng) {
			*out++ = tokendelim; *out = 0;
			strcat(out-1, word+stemleng); n++;
