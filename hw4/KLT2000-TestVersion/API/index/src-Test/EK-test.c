...
//	} else WELCOME(mode.version);	/* welcome message: 'header/ham-ndx.h' */
	...
	/*========================= 2. Extract Terms ===========================*/
	while (!feof(fpin) && fscanf(fpin, "%s", sent)) {
		if (ispunct(sent[strlen(sent)-1])) sent[strlen(sent)-1] = 0;	// 문장부호 제거

/*		if (mode.autospac) {
			n = adjust_word_spacing_nsyl(sent, temp, SENTSIZE, mode.autospac);	// 자동 띄어쓰기 -- 1: sent의 blank 유지, 0: sent의 blank 무시
			printf("%s\n%s(%d)\n", sent, temp, n);
		}
*/
		if ((sent[0] & 0x80) && sent[0] >= 0xCA) {	// 한자 -> 한글 변환
			HANJA2KSC(sent, temp);
			printf("%s -> %s\n", sent, temp);
		} else if (sent[0] & 0x80) {
			if (convert_k2e(sent, temp))	// 한글 -> 영문  변환
				printf("%s -> %s\n", sent, temp);
		} else {
			if (convert_e2k(sent, temp))	// 영문 -> 한글 변환
				printf("%s -> %s\n", sent, temp);
		}
	}
	/*======================================================================*/
