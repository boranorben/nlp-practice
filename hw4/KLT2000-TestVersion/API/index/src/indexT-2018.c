...
<<주의사항>>
Linux/Unix에서 컴파일이 안될 때 -- header/ham-api.h의 아래 라인을 commemt처리
	//#define WINDOWS_DLL
--------------------------------------------------------------------------*/
#include <stdlib.h>
...
FILE *fp;
{
	...
	p = (unsigned char *) malloc(n+100);	/* memory allocation */
	if (p == NULL) return NULL;

	memset(p, 0, n+100);	// 2014/11/12 추가 -- 강승식
	fseek(fp, 0L, 0);
...
main(int argc, char *argv[])
{
	...
	if (argc > 1 && argv[1][0] == '-')
		n2 = atoi(argv[1]+1);	/* 출력할 term의 최대 개수 지정 */
...
//	n = get_terms_text(text, Term, &TM, &mode, n2, 3, 1);
	n = get_terms_text_simple(text, Term, &TM, &mode, n2, 3, 1, optstr, fname_INI);
	/*------------------------------------------------------------------------
