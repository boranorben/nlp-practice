...
#include "../header/get-tok.h"

extern PREFIX int get_tokens_TS(
	HAM_PUCHAR word,	/* input stream -- KSC5601 or UTF8 */
	int length,			/* byte-length of an input */
	TOKEN_STR out[],	/* ptr. array of keywords */
	HAM_PMORES hamout,	/* morph. analysis result */
	HAM_PRUNMODE mode);	/* running mode of HAM */

extern PREFIX unsigned char *UTF2KSC();
extern PREFIX unsigned char *KSC2UTF();
...
void put_tokens(FILE *fp, unsigned char *word, TOKEN_STR out[], int n, HAM_RUNMODE *mode)
{
	int i;
	...
	for (i=0; i < n; i++) {
		strncpy(tok, out[i].token, out[i].length); tok[out[i].length]=0;
		fprintf(fp, "\t%s", tok);
	}
	fprintf(fp, "\n");
}
...
int argc; char *argv[];
{
	...
	unsigned char word[SENTSIZE];	/* input word or sentence */
...
*/
	...
	if ((ch = getc(fpin)) == 0xEF) {	// UTF8 인코딩 확인 -- EF/BB/BF
		if ((ch = getc(fpin)) == 0xBB) {
			if ((ch = getc(fpin)) == 0xBF) {
				mode.hcode_in = mode.hcode_out = 2;	// UTF8 encoding
				putc(0xEF, fpout); putc(0xBB, fpout); putc(0xBF, fpout);	// BOM -- UTF8 header
			} else ungetc(ch, fpin);
		} else ungetc(ch, fpin);
	} else ungetc(ch, fpin);

	if (optstr && *optstr == 'O' && isdigit(*(optstr+1))) {	// term extraction output option
		mode.index.verbs = *(optstr+1) - '0';
		if (*(optstr+2) == '0') mode.hcode_out = 1;	// KSC5601 output for testing
	}

	mode.index.stopw = 0;	// stopword.dic -- apply(1) or not(0)
	/*========================= 2. Extract Terms ===========================*/
	while (!feof(fpin) && fscanf(fpin, "%s", word) == 1) {
		if (word[0] == '\0') break;
		word[SENTSIZE-1] = '\0';
//		n = get_stems(word, term, &mode);	/* It is NOT thread-safe!!! */
//		n = get_stems_TS(word, term, KeyMem, &HamOut, &mode);	/* thread-safe version of 'get_stems()' */
//		put_stems(fpout, word, term, n, &mode);
		n = get_tokens_TS(word, strlen(word), out, &HamOut, &mode);	/* thread-safe version of 'get_stems()' */
		put_tokens(fpout, word, out, n, &mode);
	}
	/*======================================================================*/
