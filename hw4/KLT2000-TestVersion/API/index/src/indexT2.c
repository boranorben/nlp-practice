/*==========================================================================
	...
	1. 실행파일의 기능
		n개의 문서들에 대해 순서대로 색인어를 추출하여 결과를 stdout으로 출력
		- 문서파일(text file)들이 1개의 디렉토리에 저장되어 있어야 함.
		- 문서파일들에 대한 파일이름을 1줄에 1개씩 나열한 파일이 있어야 함.

	2. 입력 -- 실행하면 아래 2가지를 물어보게 되어 있음
		1) 문서 파일들이 들어 있는 디렉토리 경로
		2) 문서 파일명이 1줄에 1개씩 나열된 파일 이름

	3. 출력 -- 표준 출력파일(stdout)으로 결과를 출력
		(출력 결과는 각 line마다 아래 순서로 되어 있음)

		<색인어, 문서번호, 가중치, 출현빈도, 위치정보_1, 위치정보_2, 위치정보_n>

		- '색인어' -- 문서에서 추출된 색인어 문자열로서 주로 명사들만 추출함.
		- '문서번호' -- 파일에 나열된 문서(n개)를 순서대로 0부터 n-1까지로 표현하였음.
		- '가중치' -- 0~100 사이의 값으로서 값이 클수록 해당 문서에서 색인어의 중요도가 큼.
		- '출현빈도' -- '색인어'가 해당 문서에 출현한 횟수
		- '위치정보' -- '색인어'가 해당 문서에서 출현한 어절의 순서를 0부터 m까지로 표현.

	2003년 10월 국민대학교 컴퓨터학부 강승식 sskang@kookmin.ac.kr
...
*/
	for (i = 0; i < n; i++) {
		...
		fprintf(fp, "\t%5u", term[i].tf);	/* 용어의 출현빈도 */
...
main(int argc, char *argv[])
{
	...
	char fname[999];	/* input file name -- dir. path + file name */
	...
	fprintf(stderr, "Directory path where doc. files are stored(ex. c:\\kang\\docs\\) : "); scanf("%s", fname);
	fprintf(stderr, "A file name that doc. file names are listed(ex. DocNames.lst) : "); scanf("%s", filename);
	...
	while (fgets(p_fname, 999, fp)) {
		...
		/*=========================== Extract Terms ============================*/
		n = get_terms_text(text, Term, &TM, &mode, n, 1, 0);
		/*------------------------------------------------------------------------
